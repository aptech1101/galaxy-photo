...
fetch("images/images.json")
  ...
  .then((imageList) => {
    heartImages = imageList.map((name) => `images/${name}`);
    // Sau khi có danh sách ảnh, gọi hàm khởi tạo galaxy hoặc reload scene
    initGalaxyWithImages(heartImages);
  });

function initGalaxyWithImages(heartImages) {
  // ---- TẠO THIÊN HÀ (GALAXY) ----
  const galaxyParameters = {
    count: 100000,
    arms: 6,
...
console.log("Danh sách heartImages:", heartImages);
